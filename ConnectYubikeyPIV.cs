...
namespace Yubikey_Powershell
{
    ...
    public class ConnectYubikeyPIVCommand : Cmdlet
    {
        ...
        {
            // If already connected disconnect first
            if (YubiKeyModule._connection is not null) { YubiKeyModule._connection.Dispose(); }
            if (YubiKeyModule._yubikey is null) { throw new Exception("No YubiKey connected, use Connect-YubiKey first"); }
            try
            {
                YubiKeyModule._connection = YubiKeyModule._yubikey.Connect(YubiKeyApplication.Piv);
            }
            catch (Exception e)
            {
                throw new Exception("Could not connect to Yubikey PIV", e);
            ...
            try
            {
                InitializeAuthenticateManagementKeyCommand auth = new InitializeAuthenticateManagementKeyCommand(true);
                InitializeAuthenticateManagementKeyResponse authResponse = YubiKeyModule._connection.SendCommand(auth);
                if (authResponse.Status != ResponseStatus.Success)
                {
                    throw new Exception("Could not authenticate to YubiKey");
                }
                WriteDebug("Initial Authentication");
                //convert management key from string to byte[]
                byte[] mgmtKey = HexConverter.StringToByteArray(ManagementKey);
                CompleteAuthenticateManagementKeyCommand completeAuth = new CompleteAuthenticateManagementKeyCommand(authResponse, mgmtKey);
                CompleteAuthenticateManagementKeyResponse completeAuthResponse = YubiKeyModule._connection.SendCommand(completeAuth);
                if (authResponse.Status != ResponseStatus.Success)
                {
                    throw new Exception("Could not authenticate Yubikey PIV, wrong ManagementKey");
                }
                CryptographicOperations.ZeroMemory(mgmtKey);
            }
            catch (Exception e) { new Exception("Could not authenticate Yubikey PIV, wrong ManagementKey", e); }
                byte[] pinarray = System.Text.Encoding.UTF8.GetBytes(PIN);
                WriteDebug("Before Pin auth");
                VerifyPinCommand pinauth = new VerifyPinCommand(pinarray);
                VerifyPinResponse pinauthResponse = YubiKeyModule._connection.SendCommand(pinauth);
                WriteDebug("Post PIN Auth");
                WriteDebug($"Response: {pinauthResponse.Status}");
                CryptographicOperations.ZeroMemory(pinarray);
                if (pinauthResponse.Status == ResponseStatus.Success)
                {
                    WriteObject("Success");
                }
                else if (pinauthResponse.Status == ResponseStatus.AuthenticationRequired)
                {
                    WriteDebug("Wrong PIN provided to authenticate to YubiKey");
                    throw new Exception("Wrong PIN provided to authenticate to YubiKey");
                }
                else
                {
                }
                WriteDebug("Initial Authentication complete");
