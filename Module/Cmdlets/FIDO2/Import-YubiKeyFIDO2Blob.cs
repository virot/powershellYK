...
/// When no credential or relying party is provided, the cmdlet automatically creates or reuses
/// a synthetic FIDO2 credential (RP and username both set to "blob-storage") and uses it for storage.
///
/// .EXAMPLE
/// Import-YubiKeyFIDO2Blob -LargeBlob .\test.txt
/// Imports a file as a large blob, automatically creating or reusing a "blob-storage" credential.
///
/// .EXAMPLE
/// Import-YubiKeyFIDO2Blob -LargeBlob .\test.txt -Force
/// Same as above but skips the credential-creation confirmation prompt.
///
/// .EXAMPLE
/// Import-YubiKeyFIDO2Blob -LargeBlob .\test.txt -RelyingPartyID "demo.yubico.com"
/// Imports a file as a large blob when there is no more than one credential for the Relying Party on the YubiKey.
///
/// .EXAMPLE
/// Import-YubiKeyFIDO2Blob -LargeBlob .\test.txt -CredentialId "19448fe...67ab9207071e"
/// Imports a file as a large blob for a specified FIDO2 credential by ID (use when the RP has multiple credentials).
///
/// .EXAMPLE
/// Import-YubiKeyFIDO2Blob -LargeBlob .\test.txt -CredentialId "19448fe...67ab9207071e" -Force
...
{
    [Cmdlet(VerbsData.Import, "YubiKeyFIDO2Blob", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.High, DefaultParameterSetName = "AutoCreate")]
    public class ImportYubikeyFIDO2BlobCmdlet : PSCmdlet
    {
        // Parameters for large blob import
        [Parameter(
            Mandatory = true,
            ParameterSetName = "AutoCreate",
        ...
        [Parameter(
            ...
            ParameterSetName = "AutoCreate",
            ValueFromPipeline = false,
            HelpMessage = "Suppress confirmation prompts (credential creation and blob overwrite)."
        )]
        [Parameter(
            Mandatory = false,
            ParameterSetName = "Set LargeBlob",
        ...
        private const string AutoCreateUsername = "blob-storage";
        ...
        protected override void ProcessRecord()
        {
            ...
            if (ParameterSetName == "AutoCreate")
            {
                ...
                using (var lookupSession = new Fido2Session((YubiKeyDevice)YubiKeyModule._yubikey!))
                {
                    ...
                    if (lookupSession.AuthenticatorInfo.MaximumSerializedLargeBlobArray is null)
                    {
                    ...
                    if (match is not null)
                    {
                        ...
                        if (creds.Count > 0)
                        {
                            ...
                            WriteDebug($"AutoCreate: reusing existing credential {Convert.ToHexString(credIdBytes).ToLowerInvariant()} for RP '{AutoCreateRpId}'.");
                        }
                    }
                }
                ...
                if (credIdBytes is null)
                {
                    if (!Force.IsPresent && !ShouldContinue(
                        $"No credential or relying party was provided. A new FIDO2 credential will be created on RP '{AutoCreateRpId}'. Continue?",
                        "Create FIDO2 credential"))
                    {
                        return;
                    }

                    WriteDebug($"AutoCreate: invoking New-YubiKeyFIDO2Credential for RP '{AutoCreateRpId}', user '{AutoCreateUsername}'...");
                    var ps = PowerShell.Create(RunspaceMode.CurrentRunspace)
                        .AddCommand("New-YubiKeyFIDO2Credential")
                        .AddParameter("RelyingPartyID", AutoCreateRpId)
                        .AddParameter("Username", AutoCreateUsername)
                    ...
                    if (results.Count == 0 || results[0].BaseObject is not CredentialData credData)
                        throw new InvalidOperationException("Failed to create a FIDO2 credential for blob storage.");
                ...
                selectedCredentialId = new powershellYK.FIDO2.CredentialID(credIdBytes);
            }
            else if (ParameterSetName == "Set LargeBlob by RelyingPartyID")
            {
                ...
                using (var resolveSession = new Fido2Session((YubiKeyDevice)YubiKeyModule._yubikey!))
                {
                    ...
                    try
                    {
                        ...
                        resolvedRpId = credentialRelyingParty.Id;
                    }
            ...
            {
                // "Set LargeBlob" — CredentialId provided directly
                ...
                // resolvedRpId stays null; Phase 2 will scan for it
            }

            // ── Phase 2: Fresh session for assertion + blob operations ──
            using (var fido2Session = new Fido2Session((YubiKeyDevice)YubiKeyModule._yubikey!))
            {
                ...
                WriteDebug($"Step 2: Large blob support verified (max {fido2Session.AuthenticatorInfo.MaximumSerializedLargeBlobArray.Value} bytes).");

                // Resolve the RelyingParty object for the assertion
                RelyingParty credentialRelyingParty;
                if (resolvedRpId is not null)
                {
                    credentialRelyingParty = new RelyingParty(resolvedRpId);
                }
                else
                {
                    // CredentialId-only: scan all RPs to find the matching one
                    RelyingParty? foundRp = null;
                    ...
                    credentialRelyingParty = foundRp;
                }
