...
namespace VirotYubikey.Cmdlets.Other
{
    ...
    public class ConfirmYubikeyAttestionCommand : Cmdlet
    {
        ...
        private static string pemFormatRegex = "^-----BEGIN CERTIFICATE(| REQUEST)-----(?<certificateContent>.*)-----END CERTIFICATE(| REQUEST)-----$";
        ...
        {
            Regex regex = new Regex(pemFormatRegex, RegexOptions.Multiline);
            Match? match;
            ...
            if (CertificateRequest is not null)
            {
                if (CertificateRequest.BaseObject is string)
                {
                    ...
                    match = regex.Match((string)CertificateRequest!.BaseObject.ToString().Replace("\r\n", "").Replace("\n", "").Replace("\r", ""));
                    if (match.Success)
                    {
            ...
            if (AttestionCertificate is not null) {
                if (AttestionCertificate.BaseObject is string)
                {
                    ...
                    match = regex.Match((string)AttestionCertificate!.BaseObject.ToString().Replace("\r\n", "").Replace("\n", "").Replace("\r", ""));
                    if (match.Success)
                    {
            ...
            if (IntermediateCertificate is not null)
            {
                if (IntermediateCertificate.BaseObject is string)
                {
                    ...
                    match = regex.Match((string)IntermediateCertificate.BaseObject.ToString().Replace("\r\n", "").Replace("\n", "").Replace("\r", ""));
                    if (match.Success)
                    {
            ...
            bool attestionComplete = chain.Build(_AttestionCertificate);
            ...
            foreach (X509Extension extension in _AttestionCertificate.Extensions)
            {
                switch (extension.Oid!.Value)
                {
                    case "1.3.6.1.4.1.41482.3.3": // Firmware version
                        ...
                        returnObject.FirmwareVersion = new FirmwareVersion(extension.RawData[0], extension.RawData[1], extension.RawData[2]);
                        break;
                    case "1.3.6.1.4.1.41482.3.7": // Serial number
                        ...
                        returnObject.SerialNumber = BitConverter.ToUInt32(tempSerialBytes, 0);
                        break;
                    case "1.3.6.1.4.1.41482.3.8": // Pin / Touch Policies
                        WriteDebug("Extracting Pin / Touch Policies");
                        returnObject.PinPolicy = (PivPinPolicy)extension.RawData[0];
                        returnObject.TouchPolicy = (PivTouchPolicy)extension.RawData[1];
                        break;
                    case "1.3.6.1.4.1.41482.3.9": // Form factor
                        WriteDebug("Extracting Form factor");
                        returnObject.FormFactor = (FormFactor)(extension.RawData[0] & (byte)0x7F);
                        break;
                }
            }
            foreach (X509Extension extension in _IntermediateCertificate.Extensions)
            {
                switch (extension.Oid!.Value)
                {
                    case "1.3.6.1.4.1.41482.3.10":
                        ...
                        returnObject.isFIPSSeries = true;
                        break;
                    case "1.3.6.1.4.1.41482.3.11":
                        WriteDebug("Yubikey is CSPN series");
                        returnObject.isCSPNSeries = true;
                        break;
                }
            }
            ...
            {
                returnObject.AttestionMatchesCSR = _AttestionCertificate.PublicKey.EncodedKeyValue.RawData.SequenceEqual(_CertificateRequest.PublicKey.EncodedKeyValue.RawData);
            }
